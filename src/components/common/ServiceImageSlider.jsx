...
import LandingLink, { isInternalRoute } from './LandingLink'
...
function ServiceCard({ service, index }) {
  ...
  const className =
    'group/card relative block h-40 w-[150px] shrink-0 overflow-hidden border-r border-master-plum/10 bg-master-plum shadow-sm transition hover:border-master-gold/35 hover:shadow-md sm:h-44 sm:w-[175px] md:w-[190px]'
  ...
  return (
    <a key={`${service.label}-${index}`} href={href} className={className} aria-label={service.label}>
      {content}
    </a>
  )
}
    ...
    <div className="group/slider relative w-full overflow-hidden">
      <div className="services-marquee flex w-max">
        {LOOP_ITEMS.map((service, i) => (
          <ServiceCard key={`${service.label}-${i}`} service={service} index={i} />
        ))}
      </div>
    </div>
  )
